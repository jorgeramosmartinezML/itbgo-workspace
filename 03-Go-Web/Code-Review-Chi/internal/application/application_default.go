...
func (a *ServerChi) Run() (err error) {
	...
	// - validator
	//vl := validator.NewVehicleValidator()
	//svVl := service.NewVehicleServiceValidate(sv, vl)
	// - handler
	...
	rt.Route("/vehicles", func(rt chi.Router) {
		...
		rt.Get("/color/{color}/year/{year}", hd.GetAllByColorAndYear())
		// - GET /vehicles/brand/{brand}/year/{start_year}/{end_year}
		rt.Get("/brand/{brand}/between/{start_year}/{end_year}", hd.GetAllByBrandAndYearRange())
		// - GET /vehicles/average_speed/brand/{brand}
		rt.Get("/average_speed/brand/{brand}", hd.GetAverageSpeedByBrand())
		// - POST /vehicles/batch
		rt.Post("/batch", hd.CreateAll())
		// - PATCH /vehicles/{id}/update_speed
		rt.Patch("/{id}/update_speed", hd.UpdateMaxSpeed())
		// - GET /vehicles/fuel_type/{type}
		rt.Get("/fuel_type/{type}", hd.FindAllByFuelType())
		// - DELETE /vehicles/{id}
		rt.Delete("/{id}", hd.DeleteById())
		// - GET /vehicles/transmission/{type}
		rt.Get("/transmission/{type}", hd.FindAllByTransmission())
		...
		rt.Get("/dimensions", hd.FindAllByDimensionsRange())
		// - GET /vehicles/weight?min={min_weight}&max={max_weight}
		rt.Get("/weight", hd.FindAllByWeightRange())
